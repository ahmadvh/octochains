...
That's a full parallel-isolated reasoning pipeline — no custom classes, no prompt engineering. See the [Official Preset Agents](#official-preset-agents) catalog for all 11 available specialists. Everything below shows how to build your own agents, aggregators, and Skills from scratch.
...
For dissecting the same log and endpoint evidence from non-overlapping security angles in parallel.
...
| `insider_threat_analyst` | Insider Threat Analyst | Authenticated-user misuse — baseline deviations, exfiltration signals, negligence vs. malicious intent |
